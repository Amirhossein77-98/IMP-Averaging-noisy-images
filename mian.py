...
def generate_pdf_report(samples):
    ...
    doc = SimpleDocTemplate("result/results.pdf", pagesize=landscape(A5))
    elements = []

    # Add the original image with title
    original_image = Image("original.jpg", width=3*inch, height=2*inch)
    ...
    for i in range(0, len(image_files), 3):
        ...
        for j in range(3):
            if i + j < len(image_files):
                image = Image(image_files[i + j], width=2*inch, height=1.5*inch)
